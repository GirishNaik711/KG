...
Reference skill the `feature-implementer` consults during the AAH build phase when an
...
**AAH mode — the `feature-implementer` runs this unattended in a git worktree. It is NOT
...
|---|---|
| Whether tools use MCP at all | **`mcp-integration` (aah-discuss)** tool-protocol-registry — `mcp-standard` means MCP is in scope (other options mean skip this skill) |
...
| Agent framework | **`agent-framework` (aah-discuss)** framework-selection |
...
build-phase `feature-implementer` does not deploy or ask for a profile. The items below apply
...
**AAH mode: skip the raw CLI walkthrough below — use the harness script instead**
(`aah run core.deploy.agentcore_deploy {scaffold|deploy|status}`, per "Harness integration"
above). The raw `agentcore create`/`agentcore deploy` steps that follow are for **standalone**
use only, where there's no harness script to call. Typing the raw CLI in AAH mode races the
harness's own idempotency ledger and duplicates work.

### Deploy via AgentCore CLI (standalone mode only)
