#!/usr/bin/env python3
"""Single source of truth for the PROJECT-level delivery ruleset.

This module holds ``PROJECT_CLAUDE_MD`` — the delivery-framework instructions
that are stamped into every scaffolded project's ``CLAUDE.md``. It is the ONLY
copy of the project ruleset; the repo-root ``CLAUDE.md`` keeps HARNESS-level
rules only.

The constant is a ``str.format()`` template: the header line interpolates
``{project_name}`` and ``{stack}``; the rules body is identical across all
projects. Any literal ``{``/``}`` in the body is escaped as ``{{``/``}}`` so
``.format()`` leaves it intact.

The state-directory name is hard-coded to ``.aah/`` to match the scaffolder
(``aah.core.scaffold.rapids_dir``), which writes ``.aah/``.

Brownfield writes wrap the rendered ruleset in these markers so a re-scaffold
replaces the block idempotently instead of duplicating it.
"""

AAH_BLOCK_BEGIN = "<!-- AAH:BEGIN -->"
AAH_BLOCK_END = "<!-- AAH:END -->"


PROJECT_CLAUDE_MD = """\
# AAH Delivery Project — {project_name}

This project ({project_name}, stack: {stack}) uses the AAH (Ascend Agentic
...
def render_project_claude_md(project_name: str, stack: str | None = None) -> str:
    """Render the project ruleset for ``project_name`` / ``stack``.

    ``stack`` falls back to a neutral placeholder when not provided (brownfield
    projects with no ``stack_choices.primary`` in the manifest, or greenfield
    scaffolds invoked without ``--stack``).
    """
    return PROJECT_CLAUDE_MD.format(
        project_name=project_name,
        stack=stack or "<unspecified>",
    )


def wrap_aah_block(body: str) -> str:
    """Wrap a rendered ruleset in the idempotent AAH block markers."""
    return f"{AAH_BLOCK_BEGIN}\n{body.rstrip()}\n{AAH_BLOCK_END}\n"
